...
import { GetRepairOrderForStaff } from '../../../../../request/getRepairOrder';
import { Card, Box, Button, Typography } from '@material-ui/core';

const ManagerOrderByStaffPage = () => {
  const [token, setToken] = React.useState(localStorage.getItem('token'));
  // const [openRepair, setOpenRepair] = React.useState(false);
  const { adminId } = useParams();
  const { page, pageSize } = usePagination();
  const { data, refetch, isLoading, error, isSuccess } = GetRepairOrderForStaff(
    {
      adminId,
      page,
      pageSize,
...
export default compose(withPaginate)(ManagerOrderByStaffPage);
